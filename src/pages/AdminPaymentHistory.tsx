...
const AdminPaymentHistory = () => {
  ...
  const buildReceiptData = (paymentId: string) => {
    ...
    return {
      receiptNo,
      date: receiptDate,
      studentName: p.applications?.student_name || "—",
      level: levelLabels[p.applications?.education_level || ""] || p.applications?.education_level || "—",
      classGrade: p.applications?.class_grade || null,
      schoolName: getSchoolName(p.applications?.school_id || null),
      parentName: p.applications?.parent_name || "—",
      parentPhone: p.applications?.parent_phone || "—",
      paymentCode: p.payment_codes?.code || null,
      isVerified: !!p.payment_codes,
      applicationFormFee: Number(p.amount),
      lawyerFormFee: 0,
      totalFees: Number(p.amount),
      orgName: "God's Will Scholarship Fund",
      orgAddress: "Kampala, Uganda",
      orgPhone: "+256 700 000000",
      orgEmail: "info@godswill.org",
      logoText: "GW",
      footerNote: "This receipt confirms payment. Keep for your records.",
      signatureName: "Administrator",
      signatureTitle: "Program Director",
      seasonalRemark,
      qrData: JSON.stringify({ receipt: receiptNo, student: p.applications?.student_name, amount: p.amount, appId: p.application_id }),
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
                ...
                <TableRow key={p.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center gap-1 justify-end">
                      <Button size="icon" variant="ghost" onClick={() => setPreviewPaymentId(p.id)} title="Preview receipt">
                        <Eye size={15} />
                      </Button>
                      <Button size="icon" variant="ghost" onClick={() => setPreviewPaymentId(p.id)} title="Print receipt">
                        <Printer size={15} />
                      </Button>
                    </div>
                  </TableCell>
      ...
      <ReceiptPreview
        ...
        data={previewPaymentId ? buildReceiptData(previewPaymentId) : null}
      />
    </div>
  );
};
