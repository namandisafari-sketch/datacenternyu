...
import { Link2, Copy, CheckCircle, Clock, Eye, Calendar, MessageCircle, Plus } from "lucide-react";
...
const AdminBursaryRequests = () => {
  ...
  const approveAndCreateAppointment = async () => {
    ...
    const { data: apptData, error: apptError } = await supabase
      .from("appointments")
      .insert({
        person_name: selectedReq.full_name,
        phone: selectedReq.phone,
        appointment_date: appointmentDate,
        seat_number: seatNumber,
        purpose: "bursary_request",
        requirements,
        notes: appointmentNotes,
        bursary_request_id: selectedReq.id,
        created_by: user!.id,
      })
      .select("id")
      .single();
    ...
    await supabase
      ...
      .update({
        ...
        admin_notes: appointmentNotes,
      })
      .eq("id", selectedReq.id);

    // Generate WhatsApp message
    const dateFormatted = new Date(appointmentDate).toLocaleDateString("en-UG", {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Link2 className="h-6 w-6 text-primary" /> Bursary Requests
          </h1>
          <p className="text-muted-foreground text-sm">Generate one-time links and review submissions</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Submissions ({requests.length})</CardTitle>
        </CardHeader>
        <CardContent>
                ...
                {requests.length === 0 ? (
                  ...
                  requests.map((req) => (
                    <TableRow key={req.id}>
                      ...
                      <TableCell className="capitalize">{req.education_level?.replace("_", " ") || "—"}</TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          {req.status === "pending" && (
                            <>
                              <Button size="sm" variant="default" onClick={() => openApproveDialog(req)} className="gap-1 text-xs">
      ...
      <Dialog open={showApproveDialog} onOpenChange={setShowApproveDialog}>
        <DialogContent className="max-w-md">
          ...
          {selectedReq && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input value={seatNumber} onChange={(e) => setSeatNumber(e.target.value)} placeholder="e.g. A1, 15" maxLength={10} />
              </div>
              <div className="space-y-2">
                <Label>Notes (optional)</Label>
                <Textarea value={appointmentNotes} onChange={(e) => setAppointmentNotes(e.target.value)} rows={2} maxLength={500} />
              </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={approveAndCreateAppointment} className="gap-2">
              <MessageCircle className="h-4 w-4" /> Approve & Send WhatsApp
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
